...
from dataguard.storage.json_codec import dumps_snapshot, loads_snapshot


class StorageError(RuntimeError):
    """Raised when local DataGuard state cannot be read or written."""
...
class SnapshotStore:
    ...
    def __init__(self, root: Path | None = None) -> None:
        self._root = root or Path(".dataguard")
    ...
    def read_baseline(self) -> ProjectSnapshot:
        ...
        if not path.exists():
            raise StorageError(
                f"Baseline snapshot '{path}' was not found. Run 'dataguard snapshot' first."
            )
        try:
