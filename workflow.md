...
They can choose a starter template with:
- `verixa init --warehouse bigquery`
...
Snowflake can use `warehouse.connection_name` or explicit account/user settings.

### 3. Validate Contracts
The user runs `verixa validate` against live warehouse data.
...
### 4. Capture Baseline
The user runs `verixa snapshot` against live warehouse data.
...
- on Snowflake, use `verixa cost snapshot --history-window 1h` for recent warehouse usage instead of pre-run estimates
...
- `verixa cost diff` estimates the same query shape without executing the full workflow on BigQuery
- `verixa cost diff --history-window 1h` reports recent Snowflake query usage for the diff workflow tag
...
- `--environment` can select an environment-specific baseline path
- `--max-bytes-billed` can cap live query cost for the run on BigQuery
- `verixa cost check` estimates the live query shape ahead of time on BigQuery
- `verixa cost check --history-window 1h` reports recent Snowflake query usage for the check workflow tag
...
18. Initial Snowflake connector support.
19. Snowflake starter templates, query tagging, accepted-values samples, live Snowflake smoke coverage, query-history cost reporting, and deeper doctor diagnostics.
...
4. The configured warehouse connector retrieves current source observations.
...
The repository includes opt-in live smoke tests at:
- `tests/integration/test_live_bigquery_smoke.py`
- `tests/integration/test_live_snowflake_smoke.py`

Enable BigQuery live validation only when all prerequisites exist:
...
Enable Snowflake live validation only when all prerequisites exist:
...
The workspace has been validated end-to-end against:
- a real BigQuery project using a mock source table
- a real Snowflake account using mock Snowflake fixtures

Both harnesses remain opt-in by default.
