...
from dataguard.contracts.models import BaselineConfig, ProjectConfig
        ...
        if baseline is None:
            findings.append(
                Finding(
                    ...
                    message="baseline missing for source; run 'dataguard snapshot' to refresh it",
                )
            )
            continue
    ...
    return [
        Finding(
            ...
            message=(
                ...
                f"{_format_duration(age_seconds)} old; refresh it with 'dataguard snapshot'"
            ),
        )
    ]
